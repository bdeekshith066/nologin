...
db.run(`
  CREATE TABLE IF NOT EXISTS shared_content (
    ...
    duration_hours INTEGER NOT NULL,  -- Change: Separate columns for hours and minutes
...
app.post('/api/share', async (req, res) => {
  const { content, durationHours, durationMinutes } = req.body;  // Change: capture hours and minutes separately
  const timestamp = new Date().getTime();
  const link = `${timestamp}`;

  const { db } = req;
  try {
    const stmt = db.prepare('INSERT INTO shared_content (link, content, duration_hours, duration_minutes) VALUES (?, ?, ?, ?)');  // Change: include hours and minutes
...
app.get('/api/:link', async (req, res) => {
  ...
  try {
    ...
    if (!row) {
      ...
      const expirationTime = (row.duration_hours * 60 + row.duration_minutes) * 60 * 1000;  // Change: convert hours and minutes to milliseconds
...
// Start the server
app.listen(port, () => {
  // Print the link directly to the terminal
